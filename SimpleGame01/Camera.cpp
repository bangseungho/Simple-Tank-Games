...
void CCamera::RoundRotate(float fYaw)
{
	XMFLOAT3 xmf3Up = XMFLOAT3{ 0.f, 1.f, 0.f };
	XMFLOAT3 xmf3Look = Vector3::Normalize(XMFLOAT3{ m_xmf3Look.x, 0.f, m_xmf3Look.z });
	XMFLOAT3 xmf3Right = Vector3::Normalize(XMFLOAT3{ m_xmf3Right.x, 0.f, m_xmf3Right.z });
	
	if (fYaw != 0.0f)
	{
		XMMATRIX mtxRotate = XMMatrixRotationAxis(XMLoadFloat3(&xmf3Up), XMConvertToRadians(fYaw));
		m_xmf3Look = Vector3::TransformNormal(xmf3Look, mtxRotate);
		m_xmf3Right = Vector3::TransformNormal(xmf3Right, mtxRotate);
	}
}
...
void CCamera::Update(CPlayer* pPlayer, XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	...
	float fTimeLagScale = fTimeElapsed * (4.0f / 0.25f);
