...
constexpr float	cGravity = 9.80665;
constexpr float BULLETS = 30;

namespace Vector3 
{
	// XMVECTOR를 XMFLOAT3으로 저장한다.
	...
	// XMFLOAT3를 노말라이즈 혹은 비노말라이즈 한뒤 스칼라 값을 곱한다.
	...
	// 두번째 인자에 fScalar를 곱한 뒤 첫번째 인자와 더한다.
	...
	// 첫번째 인자와 두번째 인자를 외적한 결과를 노말라이즈 혹은 비노말라이즈 하여 리턴
	...
	// 첫번째 벡터와 두번째 벡터의 각도를 노말라이즈하여 리턴
	...
	{	
		XMVECTOR xmvNormalVector1 = XMVector3Normalize(XMLoadFloat3(&xmf3Vector1));
		XMVECTOR xmvNormalVector2 = XMVector3Normalize(XMLoadFloat3(&xmf3Vector2));

		return Angle(xmvNormalVector1, xmvNormalVector2);
	}

	// 첫번째 벡터를 두번째 인자 행렬 변환한다.
	...
	{
		XMMATRIX xmmvtx4x4Matrix = XMLoadFloat4x4(&xmmtx4x4Matrix);

		return TransformNormal(xmf3Vector, xmmvtx4x4Matrix);
	}
	...
	{
		XMMATRIX xmmvtx4x4Matrix = XMLoadFloat4x4(&xmmtx4x4Matrix);
		return TransformCoord(xmf3Vector, xmmvtx4x4Matrix);
	}
}
