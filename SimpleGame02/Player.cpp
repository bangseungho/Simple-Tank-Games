...
void CPlayer::Move(ULONG direction, float distance, bool updateVelocity)
{
	if (direction)
	{
		...
		if (direction & DIR_RIGHT && _type == PT_TIGER) RotateBody(0, 0.3f, 0);
		if (direction & DIR_LEFT && _type == PT_TIGER) RotateBody(0, -0.3f, 0);
...
void CPlayer::RotateBody(float x, float y, float z)
{
...
// Tiger
CTigerPlayer::CTigerPlayer(ComPtr<ID3D12Device> device, ComPtr<ID3D12GraphicsCommandList> cmdList, ComPtr<ID3D12RootSignature> rootSignature)
{
...
void CTigerPlayer::OnPrepareRender()
{
	...
	Matrix mtxRotate = XMMatrixRotationRollPitchYaw(0.f, 0.f, 0.f);
