...
void CShader::BuildObjects(ComPtr<ID3D12Device> device, ComPtr<ID3D12GraphicsCommandList> cmdList)
{
...
void CObjectsShader::BuildObjects(ComPtr<ID3D12Device> device, ComPtr<ID3D12GraphicsCommandList> cmdList)
{
	...
	for (int i = 0; i < _objectNum; ++i) {
		...
		_objects[i]->SetMesh(bodyMesh);
		_turret->SetMesh(turretMesh);
		_gun->SetMesh(gunMesh);
...
void CEnemyShader::BuildObjects(ComPtr<ID3D12Device> device, ComPtr<ID3D12GraphicsCommandList> cmdList)
{
	...
	_objects[5]->SetMesh(flyerShipMesh);
...
void CInstancingShader::BuildObjects(ComPtr<ID3D12Device> device, ComPtr<ID3D12GraphicsCommandList> cmdList)
{
	std::shared_ptr<CMesh> bodyMesh = std::make_shared<CMesh>(device, cmdList, "Models/IS7/Body.bin", false);
	std::shared_ptr<CMesh> turretMesh = std::make_shared<CMesh>(device, cmdList, "Models/IS7/Turret.bin", false);
	std::shared_ptr<CMesh> gunMesh = std::make_shared<CMesh>(device, cmdList, "Models/IS7/Gun.bin", false);
	...
	for (int i = 0; i < xValue; ++i) {
		for (int j = 0; j < yValue; ++j) {
			...
			float rad = (float(360.f) / _objectNum) * (i * xValue + j);

			_objects[i * xValue + j]->SetPosition(Vec3(100 * sin(XMConvertToRadians(rad)), 1.f, 120 * cos(XMConvertToRadians(rad))));
			_objects[i * xValue + j]->LookAt(Vec3(0.f, 1.f, 0.f), _objects[i * xValue + j]->GetUpVector());
			_objects[i * xValue + j]->_bullet->SetMesh(bulletMesh);
			_objects[i * xValue + j]->_bullet->SetShader(shader);
		}
	}

	_objects[0]->SetMesh(bodyMesh);
	_objects[0]->_child->SetMesh(turretMesh);
	_objects[0]->_child->_child->SetMesh(gunMesh);

	for (int i = 1; i < _objectNum; ++i) {
		_objects[i]->_boundingBox = _objects[0]->_mesh->_boundingBox;
	}
